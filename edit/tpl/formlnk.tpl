...
<?php
	...
	// načtení obsahu
		...
		echo "<h3>Editace textového odkazu</h3>";
	else:
		echo "<h3>Přidání textového odkazu</h3>";
	endif;
?>
	<form action="./changetbl.php" method="post" enctype="multipart/form-data">
		<fieldset>
			<label for="styl">Styl odkazu</label>
			...
			<label for="typlnk">Typ odkazu</label>
			<select name="typlnk" id="typlnk">
				<option value="i" <?php echo ($typlnk == "i" ? "selected=\"selected\"" : "")?>>interní</option>
				<option value="e" <?php echo ($typlnk == "e" ? "selected=\"selected\"" : "")?>>externí</option>
			</select>
