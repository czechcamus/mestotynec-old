		...
		if (!isset($act)) {
			...
			switch (substr($ord,0,2)) {		
				case "c1":
					...
					break;
				case "c2":
					$orderstat = "ulice ".(substr($ord,-1) == "u" ? "ASC" : "DESC");
					$imgarr["col2img".(substr($ord,-1) == "u" ? "up" : "down")] = $img2name;
					break;
				case "c3":
					$orderstat = "misto ".(substr($ord,-1) == "u" ? "ASC" : "DESC");
			...
			$table = new Dwoo_Template_File('tpl/sourcetable.tpl.html');
			$filter = new Dwoo_Template_File('tpl/filterform.tpl.html');
			$record = new Dwoo_Template_File('tpl/sourcerecord.tpl.html');
			$recpages = new Dwoo_Template_file('tpl/recpages.tpl.html');
			
			// dotaz pro vyber dat tabulky
			$query = "SELECT id, nazev, ulice, misto 
				FROM udpuvodce  
			...
			$colarr = array('nazev','ulice','misto');
			...
			$data->assign('description','seznam původců');
			...
			$data->assign('title','Aldesk 1 - seznam původců');
			...
			if (isset($savebtn)) {
				if ($savebtn == 'ano') {
					...
					@$db->makeQuery("DELETE FROM udpuvodce WHERE id=$idrec");
				}
				...
				$deletedata->assign('deletetitle',"původce ".TableColumnValue($db,'udpuvodce','nazev',$idrec));
				...
				$data->assign('copyright','Camus');
				$data->assign('keywords','úřední deska,aldesk,camus');
				$data->assign('description','smazání původce');
				$data->assign('czday',GetActualCzDay());
				$data->assign('userinfo',$userinfo);
				$data->assign('curl_aid',$loguser->curl_aid);
				$data->assign('idred',$loguser->id);
				$data->assign('menudata',$menudata);
				$data->assign('title','Aldesk 1 - smazání původce');
