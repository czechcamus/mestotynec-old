	...
	// zobrazení formuláře	
		if ($showform) {
			...
			if (!isset($savebtn) && ($act == "edit")) {
				...
				$ar = $db->makeQuery("SELECT nazev FROM $maintable WHERE id=$idrec");
			...
			if ($act == "add") {
				$typelisturl = "typelist.php?$loguser->curl_aid";
			} else {
				if (!isset($typelisturl)) {
					$typelisturl = basename($_SERVER['HTTP_REFERER']);
				} else {
					$typelisturl = urldecode($typelisturl);
				}
			}
			...
			$form = new Dwoo_Template_File('tpl/typeform.tpl.html');
			...
			$fdata->assign('typelisturl',urlencode($typelisturl));
			...
			$data->assign('description','formulář zápisu kategorie');
			...
			$data->assign('title','Aldesk 1 - formulář zápisu kategorie');
